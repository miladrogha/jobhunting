...
model = OllamaLLM(model="alpha:latest")
prompt = ChatPromptTemplate.from_template(template)
chain = prompt | model


def ask_alpha(message):
    ...
    if response.ok:
        print(f"[{ask_alpha.__name__}] [{response.status_code}] Response successfully received from the model.")
        ...
        for line in response.iter_lines(decode_unicode=True):
            if line:  # Skip empty lines
                try:
                    ...
                    print("Error decoding JSON for line:", line, e)
        
        print(f"[{ask_alpha.__name__}] Sample parsed chunk: {results[0]}")
        return results
    else:
...
if __name__ == "__main__":
    ask_alpha({"role":"user", "content":"Why sky is blue?"})
